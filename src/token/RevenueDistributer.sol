...
//     function point_history(uint256 loc) external view returns (Point);
...
//     //     claim_epoch: uint256
//     //     max_epoch: uint256
//     event Claimed(address recipient, uint256 amount, uint256 claim_epoch, uint256 max_epoch);
...
//         require(msg.sender == admin || (can_checkpoint_token && (block.timestamp > last_token_time + TOKEN_CHECKPOINT_DEADLINE)), "VotingEscrowTokenTimeLock/checkpoint-token-unauthorized");
//         _checkpoint_token();
//     }
...
//             Point memory pt = VotingEscrow(ve).point_history(_mid);
//             if (pt.ts <= _timestamp) {
//                 _min = _mid;
//             } else {
//                 _max = _mid - 1;
...
//             }
//             uint256 _mid = (_min + _max + 2) / 2;
//             Point memory pt = VotingEscrow(ve).user_point_history(user, _mid);
//             if (pt.ts <= _timestamp) {
//                 _min = _mid;
//             } else {
//                 _max = _mid - 1;
...
//         Point memory pt = VotingEscrow(ve).user_point_history(_user, epoch);
//         return convert(max(pt.bias - pt.slope * convert(_timestamp - pt.ts, int128), 0), uint256);
//     }
...
//                 Point memory pt = VotingEscrow(ve).point_history(epoch);
//                 int128 dt = 0;
//                 if (t > pt.ts) {
//                     // If the point is at 0 epoch, it can actually be earlier than the first deposit
//                     // Then make dt 0
...
//         Point memory user_point = VotingEscrow(ve).user_point_history(addr, user_epoch);
...
//                 old_user_point = user_point;
//                 if (user_epoch > max_user_epoch) {
//                     user_point = Point(0, 0, 0);
//                 } else {
//                     user_point = VotingEscrow(ve).user_point_history(addr, user_epoch);
//                 }
//             } else {
//                 int128 dt = int128(week_cursor - old_user_point.ts);
//                 uint256 balance_of = uint256(max(old_user_point.bias - dt * old_user_point.slope, 0));
...
//         _last_token_time = last_token_time / WEEK * WEEK;

//         uint256 amount = _claim(addr, voting_escrow, _last_token_time);
//         if (amount != 0) {
//             address token = token;
//             require(ERC20(token).transfer(addr, amount), "transfer failed");
